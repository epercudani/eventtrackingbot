...
func processStartEventCreation(update types.Update) {
...
func processStartEventDeletion(update types.Update)  {
...
func processEventName(update types.Update)  {
...
func processDeleteIndex(update types.Update) {
...
func processAllEvents(update types.Update)  {
	...
	var text bytes.Buffer
	if len(events) > 0 {
		text.WriteString(fmt.Sprintf("Current events for this group are:"))
...
func processCurrentEvent(update types.Update) {
	...
	smr := requests.NewSendMessageRequest()
	smr.AddChatIdInt(update.Message.Chat.Id)
	smr.AddText(fmt.Sprintf("Your current event is %s.", eventName))
	smr.DoRequest()
}